...
test('frozen 0.997.1 rollback bodies, feeds, marker absence and restore order are exact', () => {
  ...
  assert.deepEqual(authority.restoreSteps.map(({ surface, action, key }) => ({ surface, action, key })), [
    { surface: 'pages', action: 'ensure_absent', key: 'ocupathif/regional-cos/v0.997.1.json' },
    ...
    { surface: 'pages', action: 'restore_body', key: 'ocupathif/direct/win32-x64/latest.yml' },
...
test('rollback authority rejects order, body hash and restored marker drift', () => {
  ...
  windows.restoreSteps[4].sha256 = 'e'.repeat(64);
...
test('pre-mutation live rollback recheck detects capture-to-cutover drift', () => {
  ...
  const windowsDrift = structuredClone(observations);
